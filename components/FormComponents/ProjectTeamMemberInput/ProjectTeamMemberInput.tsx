...
const ProjectTeamMemberInput = ({ label, name, hint }: Props): React.ReactElement => {
    const data = [{ value: 1, text: "somte" }, { value: 2, text: "kal el" }, { value: 3, text: "timmy" }]
    const { register, control, formState: { errors } } = useFormContext();
    const { fields, append, remove } = useFieldArray({ control, name: 'projectMembers' })
    return (
        <div>
                    ...
                    <div key={field.id} className={cx("govuk-form-group lbh-form-group")}>
                        <fieldset className="govuk-fieldset">
                            ...
                            <div className="repeaterGroup">
                                <Controller name={`projectMembers.${index}.name`} render={({ field: { value, onChange } }) => (
                                    <Autocomplete name={`projectMembers.${index}.name`} label="Member Name" control={control} choices={data} onChange={onChange} value={value} width={12} error={errors?.projectMembers?.[index]?.name} />
                                ...
                                <Select name={`projectMembers.${index}.role`} label="Team Role" options={[]} required={true} register={register} error={errors?.projectMembers?.[index]?.role} />
                                <button
                                    type="button"
            ...
            <Button label="Add Team member" isSecondary={true} type="button" onClick={() => append({})} />
        </div>
    )
}
